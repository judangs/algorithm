...
int main() {
    ...
    for(int i=1; i<=N; i++) {
        cin >> A;
        connect_vertex(i, sink, A);
    }

    for(int i=1; i<=M; i++) {
        cin >> B;
        connect_vertex(source, storeidx + i, B);
    }

    for(int i=1; i<=M; i++) {
        for(int j=1; j<=N; j++) {
            cin >> C;
            connect_vertex(storeidx + i, j, C);
        }
    }
    ...
    for(int i=1; i<=M; i++) {
        for(int j=1; j<=N; j++) {
            cin >> D;
            cost[storeidx + i][j] = D, cost[j][storeidx + i] = -D;
        }
    }    

    auto answer = network_flow();
    cout << answer.first << "\n";
    cout << answer.second << "\n";
}